...
public class System_Linear_Equations extends ShareTools {
    ...
    public System_Linear_Equations(String rep) {
        fn = rep;
        fr = null;
    }
    ...
    private float[] Cramer_Method_V1(float[][] A, float[] b) {
    ...
    private float[] Cramer_Method_V2(float[][] A, float[] b) {
        fr.println("x(i) = |A(i)|/|A| for each 1<=i<=n");
        int n = A.length;
        float[] x = new float[n];
        float det = ShareTools.Determinant(A);
    ...
    public void Check_User_Input(float[][] A, float[] b) throws Exception {
        ...
        if (m == n && m == k) {
            if (fn.equals("decimal") || fn.equals("rational")) {
                ...
                File file = new File("Results/" + Name_File(n) + " " + cur.format(formatter) + ".txt");
