...
public class System_Linear_Equations_Extended extends System_Linear_Equations {
    ...
    public System_Linear_Equations_Extended(float[][] nA, float[][] nb, String method, String format, PrintWriter writer) {
    ...
    public int Count_Free_Variables(float[][] A, float[][] b, int r) {
        ...
        if (t == 1 && !Is_Zero_Row(b,r)) {
            float c = - b[r][0] / b[r][1];
            if (c % 1 == 0) {
                writer.println("exist certainly const for scalar (λ = " + (int) c + ") so we simplify the vector b");
            } else {
                writer.println("exist certainly const for scalar (λ = " + c + ") so we simplify the vector b");
            }
    ...
    public void Define_Free_Variable(float[][] A, float[][] b, int r) {
        ...
        if (m < n && (Is_Unit_Vector(A,r) || m == 1)) {
            writer.println("adding new row to the system");
            Add_Zero_Row(A,b);
            A = this.A; b = this.b;
            m = A.length;
            Write_Status_System(A,b);
        ...
        if (m <= n && Is_Zero_Row(A,r) && !Is_Linear_Dependent_Rows(A)) {
            ...
            if (d1 != -1) {
                ...
                A[r][d] = 1;
                writer.println("define new column in the vector b when x" + (d + 1) + " is free variable in R" + m + " space:");
