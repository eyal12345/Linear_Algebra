...
public class System_Linear_Equations extends ShareTools {
    ...
    public System_Linear_Equations(float[][] nA, float[] nb) {
        A = nA;
        b = nb;
        try {
            int n = A[0].length;
            ...
            File file = new File(Get_Name_File(n) + " " + cur.format(formatter) + ".txt");
    ...
    private void Write_Exercise(float[][] A, float[] b) {
        int n = A.length;
        fr.println(Get_Executive_Order(n));
        if (n == 1) {
            if (A[0][0] % 1 == 0) {
                if (A[0][0] == 1) {
                    if (b[0] % 1 == 0) {
    ...
    private void Write_Matrix(float[][] A, String fn) {
        int n = A.length;
        for (int i = 0; i < n; i++) {
            for (int j = 0; j < n; j++) {
                if ((Math.round(A[i][j] * 1000.0) / 1000.0) % 1 == 0) {
    ...
    private static String Get_Name_File(int n) {
        if (n == 1) {
            return "Linear Equation (1 Equation)(1 Unknown)";
        } else {
            return "System Linear Equations (" + n + " Equations)(" + n + " Unknowns)";
        }
    }

    // invoke executive order by quantity of equations and unknowns
    private static String Get_Executive_Order(int n) {
        if (n == 1) {
    ...
    private float[] Cramer_Method_V1(float[][] A, float[] b, String fn) {
        fr.println("implement the solution by cramer method when x(i) = |A(i)|/|A| for each 1<=i<=n");
        int n = A.length;
    ...
    private float[] Cramer_Method_V2(float[][] A, float[] b, String fn) {
        fr.println("implement the solution by cramer method when x(i) = |A(i)|/|A| for each 1<=i<=n");
        int n = A.length;
        float[] x = new float[n];
        float det = Determinant(A);
    ...
    private float[] Forward_Backward_Method(float[][] A, float[] b, String fn) {
        int n = b.length;
